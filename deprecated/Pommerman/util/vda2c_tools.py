...
def value_mix_centralized_critic_postprocessing(policy,
    ...
    teammate_num = 1  # only has one teammate mo matter 2 AI vs 2 rule or 2 AI vs 2 AI
    total_agents_num = len(policy.config["grouping"]) * 2
    other_agents_num = total_agents_num - 1
    if (pytorch and hasattr(policy, "mixing_vf")) or \
        ...
        raw_opponent_batch = [opponent_batch_list[i][1] for i in range(other_agents_num)]
        opponent_batch = []
        ...
        # grab teammates reward to get global one, replace the original one for training (like SMAC and RWARE)
        sample_batch["opponent_obs"] = np.stack([opponent_batch[i]["obs"] for i in range(other_agents_num)], 1)

        # find current agent's teammate
        for i in range(total_agents_num):
            if "agent_{}".format(i) not in other_agent_batches:
                if i % 2 == 0:  # if not odd
                    teammate_batch = opponent_batch[i]
                else:
                    teammate_batch = opponent_batch[i - 1]

        sample_batch["teammate_rewards"] = np.stack([teammate_batch["rewards"] for i in range(teammate_num)],
                                                    1)
        sample_batch["rewards"] = np.concatenate(
            (np.expand_dims(sample_batch["rewards"], axis=1), sample_batch["teammate_rewards"]), axis=1).sum(1)

        sample_batch["teammate_vf_preds"] = np.stack(
            [teammate_batch["vf_preds"] for i in range(teammate_num)],
            1)
        sample_batch["all_vf_preds"] = np.concatenate(
            (np.expand_dims(sample_batch["vf_preds"], axis=1), sample_batch["teammate_vf_preds"]), axis=1)

        sample_batch["vf_tot"] = convert_to_numpy(policy.model.mixing_value(
            convert_to_torch_tensor(sample_batch["all_vf_preds"]),
            convert_to_torch_tensor(sample_batch["obs"]),
            convert_to_torch_tensor(sample_batch["opponent_obs"])))

    else:
        # Policy hasn't been initialized yet, use zeros.
        sample_batch["opponent_obs"] = np.zeros(
            (sample_batch["obs"].shape[0], other_agents_num, sample_batch["obs"].shape[1]),
            dtype=sample_batch["obs"].dtype)
        sample_batch["vf_preds"] = np.zeros_like(
            sample_batch[SampleBatch.REWARDS], dtype=np.float32)
        sample_batch["teammate_vf_preds"] = np.zeros(
            (sample_batch["vf_preds"].shape[0], teammate_num),
            dtype=sample_batch["obs"].dtype)
        sample_batch["all_vf_preds"] = np.concatenate(
            (np.expand_dims(sample_batch["vf_preds"], axis=1), sample_batch["teammate_vf_preds"]), axis=1)
        sample_batch["vf_tot"] = convert_to_numpy(policy.model.mixing_value(
...
def value_mix_actor_critic_loss(policy: Policy, model: ModelV2,
    ...
    # add mixing_function
    opponent_vf_preds = convert_to_torch_tensor(train_batch["teammate_vf_preds"])
    vf_pred = values.unsqueeze(1)
    all_vf_pred = torch.cat((vf_pred, opponent_vf_preds), 1)
    value_tot = model.mixing_value(all_vf_pred, train_batch["obs"], train_batch["opponent_obs"])
