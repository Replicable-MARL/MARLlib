...
def value_mix_centralized_critic_postprocessing(policy,
    ...
    if (pytorch and hasattr(policy, "mixing_vf")) or \
        ...
        sample_batch["mean_all_obs"] = np.concatenate(
            (np.expand_dims(sample_batch["obs"], axis=1), sample_batch["opponent_obs"]), axis=1).mean(1)

        # grab teammates reward to get global one, replace the original one for training (like SMAC and RWARE)
        sample_batch["opponent_rewards"] = np.stack([opponent_batch[i]["rewards"] for i in range(opponent_agents_num)], 1)
        sample_batch["rewards"] = np.concatenate(
            (np.expand_dims(sample_batch["rewards"], axis=1), sample_batch["opponent_rewards"]), axis=1).sum(1)
        ...
        sample_batch["vf_tot"] = convert_to_numpy(policy.model.mixing_value(
            ...
            convert_to_torch_tensor(sample_batch["mean_all_obs"])))
        ...
        sample_batch["mean_all_obs"] = np.zeros_like(
...
def value_mix_actor_critic_loss(policy: Policy, model: ModelV2,
    ...
    state = convert_to_torch_tensor(train_batch["mean_all_obs"])
