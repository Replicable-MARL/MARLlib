...
class Torch_GRU_CentralizedCritic_Model(TorchRNN, nn.Module):
    ...
    def __init__(
        ...
        input_size = self.obs_size * self.n_agents + 5 * (self.n_agents - 1)  # obs + opp_obs + opp_act
    ...
    def central_value_function(self, obs, opponent_obs, opponent_actions):
        if len(opponent_actions.shape) != 3:  # fix dummy batch error
            opponent_actions = opponent_actions.unsqueeze(2).expand(-1, 5, 5)
        input_ = torch.cat(
            [obs, torch.flatten(opponent_obs, start_dim=1), torch.flatten(opponent_actions, start_dim=1)], 1)
