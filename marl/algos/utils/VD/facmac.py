...
def q_value_mix_actor_critic_loss(policy: Policy, model: ModelV2,
    ...
    # here the facmac loss
    action_value = values.gather(1, train_batch[SampleBatch.ACTIONS].unsqueeze(1))
    all_vf_pred = torch.cat((action_value, opponent_vf_preds), 1)
...
FACMACTorchPolicy = A3CTorchPolicy.with_updates(
    name="FACMACTorchPolicy",
    get_default_config=lambda: A2C_CONFIG,
    postprocess_fn=value_mix_centralized_critic_postprocessing,
    loss_fn=q_value_mix_actor_critic_loss,
    mixins=[ValueNetworkMixin, MixingValueMixin],
)


def get_policy_class_facmac(config_):
    if config_["framework"] == "torch":
        return FACMACTorchPolicy


FACMACTrainer = A2CTrainer.with_updates(
    name="FACMACTrainer",
    default_policy=None,
    get_policy_class=get_policy_class_facmac,
)
